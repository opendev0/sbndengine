...
public:
	...
	static void check(	const char *file,			///< source filename
						...
						const char *message = NULL	///< message to append error message
	)
	{
...
*/
			...
			default:
				...
				sprintf_s(raw_message, 100, "UNKNOWN %i", errNr);
#else
...
//		std::cerr << "\033[0;40;31m" << "GL ERROR  in   " << file << "(" << line << ")" << ": " << function << "() -> " << errorMessage << " [" << string_message << "\033[0m" << std::endl;
		std::cerr << "GL ERROR  in   " << file << "(" << line << ")" << ": " << function << "() -> " << errorMessage << " [" << string_message.c_str() << std::endl;
