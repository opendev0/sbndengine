...
const float NORMAL_MAP_CORRECTION = 0.4;
...
void main(void)
{
	...
	mat3 M = mat3(-T, -S, N);
	vec3 normal = normalize(M * (vec3(texture2D(Texture1, TexCoord)) - NORMAL_MAP_CORRECTION) );
	...
	for(int i = 0; i < LIGHT_COUNT; i++){
		...
		vec4 IDiffuse  = gl_LightSource[i].diffuse  * gl_FrontMaterial.diffuse  * max(dot(normal, lightvec[i]), 0.0) + NORMAL_MAP_CORRECTION * 0.9;
		vec4 ISpecular = gl_LightSource[i].specular * gl_FrontMaterial.specular * pow(max(dot(Reflected, Eye), 0.0), gl_FrontMaterial.shininess) + NORMAL_MAP_CORRECTION * 0.1;
