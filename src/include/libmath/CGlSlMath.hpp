...
namespace GLSL
{
					...
					)
	{
		...
		float div = 1.0f/CMath<float>::sqrt(x*x+y*y+z*z);

		x *= div;
		y *= div;
		z *= div;

		float mul = CMath<float>::PI()/180.0f;
		float c = CMath<float>::cos(angle*mul);
		float s = CMath<float>::sin(angle*mul);
	...
	{
		return GLSL::vec3(CMath<float>::abs(v.data[0]), CMath<float>::abs(v.data[1]), CMath<float>::abs(v.data[2]));
	}
